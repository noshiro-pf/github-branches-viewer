        ...
        try {
          ...
          for (
            ...
            const batchGroupResults = await Promise.all(
              batchGroup.map(async (batch) => Promise.all(
                  batch.map(async (branch) => {
                    ...
                    // Build commit data
                    const commitInfo = commitData !== null ? {
                      sha: commitData.sha,
                    ...
                    const aheadBehind = comparison !== null ? {
                    ...
                    const { checksInfo, checkStatusInfo } = checks?.check_runs ? (() => {
                      ...
                      const status: 'success' | 'failure' | 'pending' | undefined = 
                        failedChecks > 0 ? 'failure' :
                        pendingChecks > 0 ? 'pending' :
                        successfulChecks === totalChecks && totalChecks > 0 ? 'success' :
                        undefined;
      ...
      <div className={"github-branches"}>
        <div className={"github-header"}>
          ...
          >
            ...
            {refreshing ? <span className={"refresh-text"}>{"Refreshing..."}</span> : null}
          </button>
        </div>
                ...
                branches.map((branch) => (
                  ...
                  <td className={"branch-name-col"}>
                    <div className={"branch-name-wrapper"}>
                      ...
                      {branch.name === defaultBranch ? <span className={"default-badge"}>{"Default"}</span> : null}
                    </div>
                  </td>
                  ...
                  <td className={"branch-status-col"}>
                    {branch.checks !== null ? <CheckStatus
                  ...
                  <td className={"branch-ahead-behind-col"}>
                    {branch.name !== defaultBranch &&
                      ((branch.behind ?? 0) > 0 || (branch.ahead ?? 0) > 0) ? <a
                        ...
                        >
                          ...
                          {(branch.ahead ?? 0) > 0 ? <>
                              <svg
                                ...
                                width={"16"}
                              >
                                <path d={"M8.47 1.22a.75.75 0 00-1.06 0L4.66 3.97a.75.75 0 001.06 1.06L7.25 3.5v9.75a.75.75 0 001.5 0V3.5l1.53 1.53a.75.75 0 001.06-1.06L8.47 1.22z"} />
                              </svg>
                  ...
                  <td className={"branch-actions-col"}>
                    {branch.name !== defaultBranch ? <BranchActionsMenu
                        branch={branch}
                    ...
                    >
                      {loadingPage ? (
                        <>
                          ...
                          >
                            <path
                              d={"M8 0a8 8 0 0 1 8 8 .75.75 0 1 1-1.5 0A6.5 6.5 0 0 0 8 1.5a.75.75 0 0 1 0-1.5Z"}
                              fill={"currentColor"}
                            />
                          </svg>
                          {"Loading more..."}
      ...
      <>
        ...
        {isOpen ? <FloatingPortal>
            ...
            >
              ...
              <div className={"menu-item"}>
                <button className={"menu-button"} onClick={handleCopyBranchName} type={"button"}>
                  <svg
                    fill={"currentColor"}
                    height={"16"}
                    viewBox={"0 0 16 16"}
              ...
              {branch.name !== defaultBranch ? <div className={"menu-item"}>
                  <a
                    className={"menu-link"}
                    href={`https://github.com/${repository.owner}/${repository.name}/compare/${defaultBranch}...${branch.name}`}
                    rel={"noopener noreferrer"}
                  ...
                  >
                    <svg
                      ...
                      width={"16"}
                    >
                      <path d={"M1.75 1A1.75 1.75 0 000 2.75v8.5C0 12.216.784 13 1.75 13H3v1.543a.75.75 0 001.206.596L7.81 13h6.44A1.75 1.75 0 0016 11.25v-8.5A1.75 1.75 0 0014.25 1H1.75zM1.5 2.75a.25.25 0 01.25-.25h12.5a.25.25 0 01.25.25v8.5a.25.25 0 01-.25.25H7.5L4.5 13.25v-2.5H1.75a.25.25 0 01-.25-.25v-8.5z"} />
                    </svg>
                    {"Compare with "}{defaultBranch}
                  </a>
                </div> : null}

              {!branch.pullRequest && branch.name !== defaultBranch ? <div className={"menu-item"}>
                  <a
                    className={"menu-link"}
                    href={`https://github.com/${repository.owner}/${repository.name}/pull/new/${branch.name}`}
                    rel={"noopener noreferrer"}
